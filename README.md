...
The project now includes a comprehensive visualization system:

- **7+ chart generation endpoints** via Flask API
- **2 HTML view endpoints** for browser-based chart viewing
- **All original plotting functions** from the monolithic script are now available via API
- **Professional, publication-ready charts** (CIP deviations, CISS, cross-correlation, dashboards, and more)
- **Multi-currency support** (EUR, USD, GBP, JPY, CHF, SEK)
- **Base64 PNG output** for easy integration with web and reports

### Example: Get a chart from the API
...
The Flask API now provides the following endpoints (see `docs/API_DOCUMENTATION.md` for full details):

- `GET /api/charts/cip_deviations` - CIP deviations chart (all currencies)
- `GET /api/charts/bandwidth_volatility` - Bandwidth vs volatility chart
- `GET /api/charts/cip_deviation_vs_band` - CIP deviation vs neutral band charts
- `GET /api/charts/ciss_indicator` - CISS indicator chart
- `GET /api/charts/ciss_comparison` - Official vs constructed CISS comparison
